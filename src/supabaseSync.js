...
export async function savePositionsToDb(positions) {
  ...
  // remove do banco as posições bgp: que o usuário excluiu no app
  const termosAtuais = new Set(rows.map((r) => r.termo));
  const { data: existentes } = await db.from("posicoes_hedge").select("id, termo").like("termo", "bgp:%");
  const orfas = (existentes || []).filter((e) => !termosAtuais.has(e.termo));
  for (const o of orfas) {
    await db.from("alocacoes_hedge").delete().eq("posicao_id", o.id);
    await db.from("posicoes_hedge").delete().eq("id", o.id);
  }
