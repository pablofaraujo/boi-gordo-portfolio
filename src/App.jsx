...
export default function Dashboard() {
  ...
  const prices = useMemo(referencePrices, []);
  ...
  const consolidated = CONTRACTS.map((contract) => {
    ...
    return { ...contract, netContracts, totalPnL, precoAtual: prices[contract.vencimento] };
  });
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: "#f9fafb", minHeight: "100vh", padding: "20px 16px" }}>
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "10px", marginBottom: "16px" }}>
          <div>
            <div style={{ fontSize: "10px", letterSpacing: "2px", textTransform: "uppercase", color: "#9ca3af", marginBottom: "4px" }}>
              B3 · Mercado Futuro · Preços de Referência
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "4px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: "12px", color: "#6b7280" }}>Atualizado {lastUpdate.toLocaleTimeString("pt-BR")}</span>
              <button onClick={() => setLastUpdate(new Date())} style={{ background: "#fff", border: "1px solid #d1d5db", borderRadius: "6px", padding: "4px 10px", fontSize: "12px", color: "#374151", cursor: "pointer", fontFamily: "inherit" }}>
                Atualizar
              </button>
            </div>
          </div>
        ...
        <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "10px 14px", color: "#1d4ed8", fontSize: "12px", marginBottom: "16px" }}>
          Cotações online indisponíveis para estes vencimentos. O painel está usando preços médios ponderados das posições como referência.
        </div>
            ...
            return (
              <div key={vencimento} style={{ background: "#fff", borderRadius: "8px", padding: "14px 16px", border: "1px solid #e5e7eb", boxShadow: "0 1px 2px rgba(0,0,0,.05)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                  ...
                  <span style={tagStyle(row?.netContracts ?? 0)}>
                    {(row?.netContracts ?? 0) < 0 ? `V ${Math.abs(row.netContracts)}` : `C ${row?.netContracts ?? 0}`}
                  </span>
                </div>
        ...
        <div style={{ background: "#fff", borderRadius: "8px", border: "1px solid #e5e7eb", boxShadow: "0 1px 2px rgba(0,0,0,.05)", marginBottom: "16px", overflow: "hidden" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table>
              <thead><tr><th className="L">Vencimento</th><th>Direção</th><th>Contr.</th><th>Px Médio</th><th>Px Ref.</th><th>Var/@</th><th>Resultado</th></tr></thead>
              <tbody>
        ...
        <div style={{ background: "#fff", borderRadius: "8px", border: "1px solid #e5e7eb", boxShadow: "0 1px 2px rgba(0,0,0,.05)", marginBottom: "16px", overflow: "hidden" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table>
              <thead><tr><th className="L">Vencimento</th><th>Posição Líq.</th><th>Preço Ref.</th><th>Resultado</th></tr></thead>
              <tbody>
        ...
        <div style={{ marginTop: "14px", fontSize: "11px", color: "#9ca3af", textAlign: "center" }}>
          Preços de referência calculados pelo preço médio ponderado das posições · 330 arrobas/contrato
        </div>
      </div>
    </div>
  );
